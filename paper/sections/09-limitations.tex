...
rather than $-0.72$, abstaining where it lacks signal. Second, our evidence base is
three hand-authored agents with a single clear success, which is too small and too
self-authored to support general claims; the gym must grow to many agents spanning
divergence types, ideally authored independently of the featuriser, before recovery
on behavioural divergences can be claimed as a property of the method rather than of
one example. These concerns also point to the fix we found most effective:
