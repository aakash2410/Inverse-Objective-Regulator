...
trajectories. Combined with a fixed goal specification
(Section~\ref{subsec:gdj}), this property makes the deterministic, structural path of
an audit reproducible from its inputs alone; the semantic-judge path inherits the
determinism caveat discussed there.
...
abstract value, and distinct from all others. The decomposition must be fixed for
the duration of an audit, which is an architectural invariant rather than a tuning
choice: because the Laplace approximation and the divergence report are computed over
the feature dimensions this call produces, any change in the goal specification would
make two ostensibly identical audits incomparable. Earlier model versions let this be
enforced with temperature-zero sampling, but newer models such as Opus~4.8 deprecate
the temperature parameter, so the decomposition is no longer deterministic by default.
We therefore fix it by other means: the gym pins a canonical goal specification (the
ordered list $\{g_1, \ldots, g_d\}$) directly, sidestepping the decomposer, and a
production audit would cache the decomposition per declared purpose. For the
\texttt{CostMinimiserAgent},
...
confidence values, and a short natural-language explanation for each score. The
critic is subject to the same determinism caveat as the decomposer: on models that
deprecate temperature its scores are not bit-reproducible, and IOR does not yet cache
them, so the semantic path carries run-to-run variation that the deterministic
structural basis (Section~\ref{subsec:variants}) does not. The
...
planted-divergence gym (Section~\ref{sec:gym}) provides eight reference agents
...
$i$, so the \emph{ranking} of the fast-mode point estimate is related to that of a
centred column-mean of the critic scores. We measure this directly in
Section~\ref{sec:evaluation} and find the two rankings moderately correlated
(Spearman $0.77$ on a deterministic basis) but frequently disagreeing on the single
most-divergent dimension, so the IRL point estimate is not merely a relabelled mean.
Even so, the strongest claims for Bayesian IRL rest on two things a column-mean
cannot provide at all: the posterior uncertainty $\sigma_i$ and the full-mode
counterfactual treatment, in which the reference is the agent's actual alternatives
...
The GDJ is one of four interchangeable featurisers, all of which satisfy a single
...
implementation. The three alternatives exist to address an obvious objection: the
...
it achieves a mean held-out behaviour-prediction lift of $0.649$, already exceeding
...
The third alternative replaces the generative judge with a decision-model critic:
an open reproduction of the Jev typed-decision pattern~\citep{typesafeJev2026,
openJevDeberta2026}, a DeBERTa-v3-large cross-encoder that scores each (state,
action, observation) against a sub-goal in a single forward pass and returns a
calibrated probability, with no text generation. It is deterministic (a classifier
forward pass, no sampling temperature), so it removes the run-to-run variance the
temperature-deprecated generative judge cannot avoid, and it runs locally, removing
the external-API dependence entirely. Like the ensemble it preserves the semantic,
purpose-grounded basis and therefore supports divergence localisation; unlike the
generative judge it cannot itself generate sub-goals, so it must be paired with a
pinned or externally supplied goal specification (as the gym does). We reimplement
its published span-pooled read-out in our own code and load only its weights, so no
third-party inference code is executed. Section~\ref{sec:evaluation} evaluates all
four featurisers, and finds (Section~\ref{subsec:jev}) that this deterministic
critic materially improves divergence recovery over the generative judge.
