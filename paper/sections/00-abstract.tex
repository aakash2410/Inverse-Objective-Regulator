...
language-model judge, a judge ensemble, and a deterministic decision-model critic. We
contribute an open gym of three planted-divergence reference environments and report
what auditing them reveals.
...
magnitude, as the reportable quantity. Acting on the judge-reliability diagnosis, we
show that a deterministic open cross-encoder critic (an open reproduction of the Jev
typed-decision pattern, run locally) raises mean recovery from $0.08$ to $0.43$,
removes the generative judge's run-to-run variance, and turns its confident
wrong-direction failure into a safe near-zero abstention. On an eight-agent gym
spanning privacy leakage, sycophancy, reward hacking, power-seeking and safety-check
skipping, this critic recovers six of eight (mean $r = 0.55$), deterministically and
offline. A deterministic model-free basis predicts held-out behaviour above a uniform
baseline, establishing a reproducible floor. The
